...
% Toolbox(TM).
%
% This example model provides a hardware-compatible algorithm. You can implement
% this algorithm on a board using an AMD(R) Zynq(R) reference design. See
% <docid:soc_ug.example-vzSharpen_PixelStream>. 
 
%% Structure of the Example
% Computer Vision Toolbox blocks operate on an entire frame at a
...
%
% The |NoiseRemovalAndImageSharpeningHDL.slx| system is shown.
...
%%
% The Image Source block imports a grayscale image, then uses a MATLAB
...
% block. For more information, see the <docid:visionhdl_ref.bt4qnrk-1> block
% reference page.

%% Pixel-Stream HDL Model
% The Median Filter block is used to remove the salt and pepper noise. To 
% learn more, refer to the <docid:visionhdl_ref.bt4qnpy_2> block reference page.
%
...
% image. To learn more, refer to the <docid:visionhdl_ref.bt4qqzr-1> block reference page.
